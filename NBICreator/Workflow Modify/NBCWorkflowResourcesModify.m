...
- (NSArray *)prepareResourcesToModify:(NSError **)error {
    ...
    if ( ( ! _isNBI && (
                        _workflowType == kWorkflowTypeImagr ||
                        _workflowType == kWorkflowTypeCasper
                        ) ) && [_userSettings[NBCSettingsARDPasswordKey] length] != 0 ) {
    ...
    if ( ( ! _isNBI && (
                        _workflowType == kWorkflowTypeImagr
                        ) ) && [_userSettings[NBCSettingsIncludeSystemUIServerKey] boolValue] ) {
...
- (void)modifyFolders:(NSMutableArray *)modifyDictArray {
    ...
    //  /usr/local
    // --------------------------------------------------------------
    NSURL *folderUsrLocal = [_baseSystemVolumeURL URLByAppendingPathComponent:@"usr/local" isDirectory:YES];
    ...
    // --------------------------------------------------------------
    NSURL *folderLibraryLaunchAgents = [_baseSystemVolumeURL URLByAppendingPathComponent:@"Library/LaunchAgents" isDirectory:YES];
    [modifyDictArray insertObject:@{
                                    NBCWorkflowModifyFileType :   NBCWorkflowModifyFileTypeFolder,
    ...
    // --------------------------------------------------------------
    NSURL *folderLibraryLaunchDaemons = [_baseSystemVolumeURL URLByAppendingPathComponent:@"Library/LaunchDaemons" isDirectory:YES];
    [modifyDictArray insertObject:@{
                                    NBCWorkflowModifyFileType :   NBCWorkflowModifyFileTypeFolder,
    ...
    // --------------------------------------------------------------
    NSURL *folderLibraryApplicationSupport = [_baseSystemVolumeURL URLByAppendingPathComponent:@"Library/Application Support" isDirectory:YES];
    [modifyDictArray insertObject:@{
                                    NBCWorkflowModifyFileType :   NBCWorkflowModifyFileTypeFolder,
    ...
    // ------------------------------------------------------------------------------------
    NSURL *folderSystemLibraryKextExtensions = [_baseSystemVolumeURL URLByAppendingPathComponent:@"System/Library/Caches/com.apple.kext.caches/Directories/System/Library/Extensions" isDirectory:YES];
    [modifyDictArray insertObject:@{
                                    NBCWorkflowModifyFileType :   NBCWorkflowModifyFileTypeFolder,
    ...
    // --------------------------------------------------------------
    NSURL *folderSystemLibraryKextStartup = [_baseSystemVolumeURL URLByAppendingPathComponent:@"System/Library/Caches/com.apple.kext.caches/Startup" isDirectory:YES];
    [modifyDictArray insertObject:@{
                                    NBCWorkflowModifyFileType :   NBCWorkflowModifyFileTypeFolder,
...
- (BOOL)modifyFolderPackages:(NSMutableArray *)modifyDictArray error:(NSError **)error {
    ...
    if ( ! [packagesFolderURL checkResourceIsReachableAndReturnError:error] ) {
        ...
        [modifyDictArray insertObject:@{
                                        ...
                                        NBCWorkflowModifyTargetURL :  [packagesFolderURL path]
                                        } atIndex:0];
...
#pragma unused(stop)
            ...
            if ( [line hasPrefix:@"RAMDisk"] ) {
                ...
                if ( 2 < [lineArray count] ) {
                    ...
                    if ( [line hasPrefix:@"/"] ) {
                        NSURL *ramDiskURL = [self->_baseSystemVolumeURL URLByAppendingPathComponent:[line substringFromIndex:1]];
                        DDLogDebug(@"[DEBUG] Adding RAMDisk folder path: %@", [ramDiskURL path]);
...
- (BOOL)modifyRCImaging:(NSMutableArray *)modifyDictArray nbiToolPath:(NSString *)nbiToolPath error:(NSError **)error {
    ...
    if ( [rcImaging length] != 0 ) {
        ...
        if (
            [_creationTool isEqualToString:NBCMenuItemNBICreator] ||
            [_creationTool isEqualToString:NBCMenuItemDeployStudioAssistant]
            ) {
            rcImagingURL = [_baseSystemVolumeURL URLByAppendingPathComponent:NBCRCImagingNBICreatorTargetURL];
        } else if (
                   [_creationTool isEqualToString:NBCMenuItemSystemImageUtility]
                   ) {
        ...
        NSDictionary *rcImagingAttributes = [[NSFileManager defaultManager] attributesOfItemAtPath:[rcImagingURL path]
                                                                                             error:nil] ?: @{
                                                                                                             NSFileOwnerAccountName :       @"root",
                                                                                                             NSFileGroupOwnerAccountName :  @"wheel",
                                                                                                             NSFilePosixPermissions :       @0644
...
- (BOOL)modifyRCInstall:(NSMutableArray *)modifyDictArray error:(NSError **)error {
    ...
    if ( [rcInstallContentStringOriginal length] != 0 ) {
        ...
        if ( [rcInstallContentString length] != 0 ) {
            ...
            NSDictionary *rcInstallAttributes = [[NSFileManager defaultManager] attributesOfItemAtPath:[rcInstallURL path]
                                                                                                 error:nil] ?: @{
                                                                                                                 NSFileOwnerAccountName :       @"root",
                                                                                                                 NSFileGroupOwnerAccountName :  @"wheel",
                                                                                                                 NSFilePosixPermissions :       @0644
...
- (BOOL)modifyScreenSharing:(NSMutableArray *)modifyDictArray error:(NSError **)error {
    ...
    NSDictionary *remoteManagementAttributes = [fm attributesOfItemAtPath:[remoteManagementURL path]
                                                                    error:nil] ?: @{
                                                                                    NSFileOwnerAccountName :      @"root",
                                                                                    NSFileGroupOwnerAccountName : @"wheel",
                                                                                    NSFilePosixPermissions :      @0644
    ...
    if ( [screensharingLaunchDaemonURL checkResourceIsReachableAndReturnError:error] ) {
        ...
        NSDictionary *screensharingLaunchDaemonAttributes = [fm attributesOfItemAtPath:[screensharingLaunchDaemonURL path]
                                                                                 error:nil] ?: @{
                                                                                                 NSFileOwnerAccountName :      @"root",
                                                                                                 NSFileGroupOwnerAccountName : @"wheel",
                                                                                                 NSFilePosixPermissions :      @0644
    ...
    if ( [screensharingAgentDaemonURL checkResourceIsReachableAndReturnError:error] ) {
        ...
        NSDictionary *screensharingAgentLaunchDaemonAttributes = [fm attributesOfItemAtPath:[screensharingAgentDaemonURL path]
                                                                                      error:nil] ?: @{
                                                                                                      NSFileOwnerAccountName :      @"root",
                                                                                                      NSFileGroupOwnerAccountName : @"wheel",
                                                                                                      NSFilePosixPermissions :      @0644
    ...
    if ( 7 < _sourceVersionMinor ) {
        ...
        if ( [screensharingMessagesAgentDaemonURL checkResourceIsReachableAndReturnError:error] ) {
            ...
            NSDictionary * screensharingMessagesAgentLaunchAgentAttributes = [fm attributesOfItemAtPath:[screensharingAgentDaemonURL path]
                                                                                                  error:nil] ?: @{
                                                                                                                  NSFileOwnerAccountName :       @"root",
                                                                                                                  NSFileGroupOwnerAccountName :  @"wheel",
                                                                                                                  NSFilePosixPermissions :       @0644
    ...
    if ( [remoteDesktopLaunchAgentURL checkResourceIsReachableAndReturnError:error] ) {
        ...
        NSDictionary *remoteDesktopLaunchAgentAttributes = [fm attributesOfItemAtPath:[remoteDesktopLaunchAgentURL path]
                                                                                error:nil] ?: @{
                                                                                                NSFileOwnerAccountName :       @"root",
                                                                                                NSFileGroupOwnerAccountName :  @"wheel",
                                                                                                NSFilePosixPermissions :       @0644
...
- (BOOL)modifySystemUIServer:(NSMutableArray *)modifyDictArray error:(NSError **)error {
    ...
    NSDictionary *systemUIServerPreferencesAttributes = [fm attributesOfItemAtPath:[systemUIServerPreferencesURL path]
                                                                             error:nil] ?: @{
                                                                                             NSFileOwnerAccountName :      @"root",
                                                                                             NSFileGroupOwnerAccountName : @"wheel",
                                                                                             NSFilePosixPermissions :      @0644
    ...
    if ( [systemUIServerLaunchAgentURL checkResourceIsReachableAndReturnError:error] ) {
        ...
        NSDictionary *systemUIServerAttributes = [fm attributesOfItemAtPath:[systemUIServerLaunchAgentURL path]
                                                                      error:nil] ?: @{
                                                                                      NSFileOwnerAccountName :      @"root",
                                                                                      NSFileGroupOwnerAccountName : @"wheel",
                                                                                      NSFilePosixPermissions :      @0644
