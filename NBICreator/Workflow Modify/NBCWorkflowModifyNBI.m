...
- (void)modifyNBI:(NBCWorkflowItem *)workflowItem {
    ...
    if ([nbiURL checkResourceIsReachableAndReturnError:&error]) {
        ...
        // ---------------------------------------------------------------
        if (!_isNBI || (_isNBI && ([_userSettingsChanged[NBCSettingsNameKey] boolValue] || [_userSettingsChanged[NBCSettingsIndexKey] boolValue] ||
                                   [_userSettingsChanged[NBCSettingsProtocolKey] boolValue] || [_userSettingsChanged[NBCSettingsEnabledKey] boolValue] ||
                                   [_userSettingsChanged[NBCSettingsDefaultKey] boolValue] || [_userSettingsChanged[NBCSettingsDescriptionKey] boolValue]))) {
            ...
            if (![self updateNBImageInfoForNBIAtURL:nbiURL workflowItem:workflowItem error:&error]) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{
                                                                      NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Updating NBImageInfo.plist failed"]
                                                                  }];
                return;
            }
        }
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                            object:self
                                                          userInfo:@{
                                                              NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"NBI doesn't exist at the expected path"]
                                                          }];
    }
} // modifyNBI
    ...
    if ([nbImageInfoDict count] != 0) {
        ...
        if ([platformSupportURL checkResourceIsReachableAndReturnError:error]) {
            ...
            if ([platformSupportDict count] != 0) {
                ...
                newNBImageInfoDict[@"DisabledSystemIdentifiers"] =
                    [[[disabledSystemIdentifiers copy] valueForKeyPath:@"@distinctUnionOfObjects.self"] sortedArrayUsingSelector:@selector(localizedCaseInsensitiveCompare:)] ?: @[];
            }
        }
        ...
        //  Adding: Name
        // ---------------------------------------------------------------
        newNBImageInfoDict[@"Name"] = [NBCVariables expandVariables:userSettings[NBCSettingsNameKey] source:source applicationSource:applicationSource] ?: @"";

        // ---------------------------------------------------------------
        //  Adding: Description
        // ---------------------------------------------------------------
        newNBImageInfoDict[@"Description"] = [NBCVariables expandVariables:userSettings[NBCSettingsDescriptionKey] source:source applicationSource:applicationSource] ?: @"";
        ...
        newNBImageInfoDict[@"Index"] = [nf numberFromString:[NBCVariables expandVariables:userSettings[NBCSettingsIndexKey] source:source applicationSource:applicationSource] ?: @"1"] ?: @1;
                ...
                if ([baseSystemVolumeURL checkResourceIsReachableAndReturnError:&error]) {
                    ...
                    [[NSNotificationCenter defaultCenter]
                        postNotificationName:NBCNotificationWorkflowFailed
                                      object:self
                                    userInfo:@{
                                        NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"BaseSystem disk image reports mounted but can't find volume path"]
                                    }];
                }
            } else {
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:NBCNotificationWorkflowFailed
                                  object:self
                                userInfo:@{
                                    NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"BaseSystem disk image is not mounted with shadow file, this state is not handled yet."]
                                }];
            }
            ...
            if ([[[_workflowItem target] nbiNetInstallShadowPath] length] != 0) {
                  ...
                  if ([NBCDiskImageController resizeAndMountBaseSystemWithShadow:[[self->_workflowItem target] baseSystemURL] target:[self->_workflowItem target] error:&error]) {
                          ...
                          dispatch_async(dispatch_get_main_queue(), ^{
                            [[NSNotificationCenter defaultCenter]
                                postNotificationName:NBCNotificationWorkflowFailed
                                              object:self
                                            userInfo:@{
                                                NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Resize and mount BaseSystem returned yes but can't find volume path"]
                                            }];
                          });
                      }
                  } else {
                      dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                            object:self
                                                                          userInfo:@{
                                                                              NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Resize and mount BaseSystem failed"]
                                                                          }];
                      });
                  }
                });
                ...
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:NBCNotificationWorkflowFailed
                                  object:self
                                userInfo:@{
                                    NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"NetInstall disk image is not mounted with shadow file, this state is not handled yet."]
                                }];
            }
        } else {
              ...
              if ([NBCDiskImageController attachNetInstallDiskImageWithShadowFile:[[self->_workflowItem target] nbiNetInstallURL] target:[self->_workflowItem target] error:&error]) {
                  ...
                  dispatch_async(dispatch_get_main_queue(), ^{
                    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                        object:self
                                                                      userInfo:@{
                                                                          NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Resize and mount NetInstall failed"]
                                                                      }];
                  });
              }
            });
        ...
        if ([[[_workflowItem target] nbiNetInstallDisk] isMounted]) {
            ...
            if ([[[_workflowItem target] nbiNetInstallShadowPath] length] != 0) {
                ...
                if ([netInstallVolumeURL checkResourceIsReachableAndReturnError:&error]) {
                    ...
                    // ------------------------------------------------------------------
                    if (([_creationTool isEqualToString:NBCMenuItemNBICreator] || [_workflowItem workflowType] == kWorkflowTypeImagr || [_workflowItem workflowType] == kWorkflowTypeCasper) &&
                    ...
                    [[NSNotificationCenter defaultCenter]
                        postNotificationName:NBCNotificationWorkflowFailed
                                      object:self
                                    userInfo:@{
                                        NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"NetInstall disk image reports mounted but can't find volume path"]
                                    }];
                }
            } else {
                [[NSNotificationCenter defaultCenter]
            ...
            dispatch_async(taskQueue, ^{
              ...
              if ([NBCDiskImageController attachNetInstallDiskImageWithShadowFile:[[self->_workflowItem target] nbiNetInstallURL] target:[self->_workflowItem target] error:&error]) {
                  ...
                  if ([netInstallVolumeURL checkResourceIsReachableAndReturnError:&error]) {
                      ...
                      //  Delete and recreate folder Packages
                      // ------------------------------------------------------------------
                      if (([self->_creationTool isEqualToString:NBCMenuItemNBICreator] || [self->_workflowItem workflowType] == kWorkflowTypeImagr ||
                      ...
                      dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter]
                            postNotificationName:NBCNotificationWorkflowFailed
                                          object:self
                                        userInfo:@{
                                            NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Attach NetInstall returned yes but can't find volume path"]
                                        }];
                      });
                  }
              } else {
...
- (void)modifyComplete {
    ...
    if ([_currentVolume isEqualToString:@"BaseSystem"]) {
        ...
        if (!_isNBI || ([_userSettingsChanged[NBCSettingsARDLoginKey] boolValue] || [_userSettingsChanged[NBCSettingsARDPasswordKey] boolValue])) {
...
- (void)modifyFailedWithError:(NSError *)error {
    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                        object:self
                                                      userInfo:@{
                                                          NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Modifying volume failed"]
                                                      }];
...
- (void)convertBaseSystemFromShadow {
    ...
    dispatch_async(taskQueue, ^{
      ...
      if ([NBCDiskImageController detachDiskImageAtPath:[baseSystemVolumeURL path]]) {
          ...
          NSURL *baseSystemDiskImageConvertedURL =
              [NSURL URLWithString:[NSString stringWithFormat:@"%@%@.%@", [baseSystemDiskImageURL URLByDeletingPathExtension], diskImageFormat, diskImageExtension]];
          ...
          if ([NBCDiskImageController convertDiskImageAtPath:[baseSystemDiskImageURL path]
                                             shadowImagePath:baseSystemShadowPath
                                                      format:diskImageFormat
                                             destinationPath:[baseSystemDiskImageConvertedURL path]]) {
...
- (void)convertNetInstallFromShadow {
    ...
    dispatch_async(taskQueue, ^{
      ...
      if ([NBCDiskImageController detachDiskImageAtPath:[netInstallVolumeURL path]]) {
          ...
          NSURL *netInstallDiskImageConvertedURL =
              [NSURL URLWithString:[NSString stringWithFormat:@"%@%@.%@", [netInstallDiskImageURL URLByDeletingPathExtension], diskImageFormat, diskImageExtension]];
          ...
          if ([NBCDiskImageController convertDiskImageAtPath:[netInstallDiskImageURL path]
                                             shadowImagePath:netInstallShadowPath
                                                      format:diskImageFormat
                                             destinationPath:[netInstallDiskImageConvertedURL path]]) {
...
#pragma unused(error)
    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                        object:self
                                                      userInfo:@{
                                                          NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Installing packages failed"]
                                                      }];
} // installFailed
...
- (void)copyFilesToVolume {
    ...
    if ([copyArray count] != 0) {
        ...
        dispatch_async(taskQueue, ^{
          ...
          }] copyResourcesToVolume:self->_currentVolumeURL
                          copyArray:copyArray
...
- (void)copyFailedWithError:(NSError *)error {
    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                        object:self
                                                      userInfo:@{
                                                          NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying files to volume failed"]
                                                      }];
} // copyFailedWithError
...
- (void)removeFolderPackagesInNetInstallVolume:(NSURL *)netInstallVolumeURL {
    ...
    if ([packagesFolderURL checkResourceIsReachableAndReturnError:nil]) {
        ...
        dispatch_async(taskQueue, ^{
          ...
          }] removeItemsAtPaths:@[ [packagesFolderURL path] ]
...
- (void)createFolderPackagesInNetInstallVolume:(NSURL *)packagesFolderURL {
    ...
    if ([[[NSFileManager alloc] init] createDirectoryAtURL:extrasFolderURL
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                            object:self
                                                          userInfo:@{
                                                              NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Creating folder Extras failed!"]
                                                          }];
    }
}
    ...
    if (!_modificationsApplied) {
        ...
        if ([modificationsArray count] != 0) {
            ...
            dispatch_async(taskQueue, ^{
              ...
              }] modifyResourcesOnVolume:self->_currentVolumeURL
                       modificationsArray:modificationsArray
                            authorization:authData
                                withReply:^(NSError *error, int terminationStatus) {
                                  ...
                                  if (terminationStatus == 0) {
                                      dispatch_async(dispatch_get_main_queue(), ^{
                                        if ((!self->_isNBI && ([[self->_workflowItem userSettings][NBCSettingsDisableWiFiKey] boolValue] ||
                                                               [[self->_workflowItem userSettings][NBCSettingsDisableBluetoothKey] boolValue])) ||
                                            (self->_isNBI &&
                                             ([self->_userSettingsChanged[NBCSettingsDisableWiFiKey] boolValue] || [self->_userSettingsChanged[NBCSettingsDisableBluetoothKey] boolValue]))) {
            ...
            if (!err) {
                dispatch_async(dispatch_get_main_queue(), ^{
                  if ((!self->_isNBI &&
                       ([[self->_workflowItem userSettings][NBCSettingsDisableWiFiKey] boolValue] || [[self->_workflowItem userSettings][NBCSettingsDisableBluetoothKey] boolValue])) ||
                      (self->_isNBI && ([self->_userSettingsChanged[NBCSettingsDisableWiFiKey] boolValue] || [self->_userSettingsChanged[NBCSettingsDisableBluetoothKey] boolValue]))) {
                      [self updateKernelCache];
                  } else {
                      [self finalizeWorkflow];
                  }
                ...
                [[NSNotificationCenter defaultCenter]
                    postNotificationName:NBCNotificationWorkflowFailed
                                  object:self
                                userInfo:@{
                                    NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:[NSString stringWithFormat:@"Modifying volume %@ failed", _currentVolume]]
                                }];
            }
        }
    } else {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
          if ((!self->_isNBI && ([[self->_workflowItem userSettings][NBCSettingsDisableWiFiKey] boolValue] || [[self->_workflowItem userSettings][NBCSettingsDisableBluetoothKey] boolValue])) ||
              (self->_isNBI && ([self->_userSettingsChanged[NBCSettingsDisableWiFiKey] boolValue] || [self->_userSettingsChanged[NBCSettingsDisableBluetoothKey] boolValue]))) {
              [self updateKernelCache];
          } else {
              [self finalizeWorkflow];
...
- (void)disableSpotlightOnVolume {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] disableSpotlightOnVolume:[self->_currentVolumeURL path]
                     authorization:authData
                         withReply:^(NSError *error, int terminationStatus) {
                           dispatch_async(dispatch_get_main_queue(), ^{
                             if (terminationStatus == 0) {
...
- (void)updateKernelCache {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] updateKernelCache:targetVolumePath
              nbiVolumePath:nbiVolumePath
...
- (void)addUsers {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] addUsersToVolumeAtPath:nbiVolumePath
                         ...
                         dispatch_async(dispatch_get_main_queue(), ^{
                           if (terminationStatus == 0) {
                               ...
                               [nc postNotificationName:NBCNotificationWorkflowFailed
                                                 object:self
                                               userInfo:@{
                                                   NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Creating user failed!"]
                                               }];
                           }
                         });
                       }];
