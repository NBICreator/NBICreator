...
- (void)partitionUSBDiskWithBSDName:(NSString *)bsdName workflowItem:(NBCWorkflowItem *)workflowItem {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] partitionDiskWithBSDName:bsdName
...
- (void)copyNBIToUSBDisk:(NBCDisk *)usbVolumeDisk workflowItem:(NBCWorkflowItem *)workflowItem {
    ...
    if ([usbResources count] != 0) {
        ...
        dispatch_async(taskQueue, ^{
          ...
          }] copyResourcesToVolume:usbVolumeURL
                          copyArray:usbResources
                      authorization:authData
                          withReply:^(NSError *error, int terminationStatus) {
                            if (terminationStatus == 0) {
                                dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)mountNetInstallDiskImage:(NBCWorkflowItem *)workflowItem usbVolumeURL:(NSURL *)usbVolumeURL {
    ...
    if ([modifyDictArray count] != 0) {
        ...
        dispatch_async(taskQueue, ^{
          ...
          }] modifyResourcesOnVolume:usbVolumeURL
                   modificationsArray:modifyDictArray
                        authorization:authData
                            withReply:^(NSError *error, int terminationStatus) {
                              if (terminationStatus == 0) {
                                  dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)blessUSBVolumeAtURL:(NSURL *)usbVolumeURL workflowItem:(NBCWorkflowItem *)workflowItem {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] blessUSBVolumeAtURL:usbVolumeURL
                        label:label
                authorization:authData
                    withReply:^(NSError *error, int terminationStatus) {
                      if (terminationStatus == 0) {
