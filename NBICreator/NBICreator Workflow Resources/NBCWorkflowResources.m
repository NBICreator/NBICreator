    ...
    if (
        ...
        if ( [packagesArray count] != 0 ) {
            if ( ! [self addInstallPackages:packagesArray error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Unable to add packages for installation"]}];
                return;
            }
        }
    }
    ...
    if (
        ...
        if ( [packagesArrayNetInstall count] != 0 ) {
            if ( ! [self addInstallPackagesNetInstall:packagesArrayNetInstall error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Unable to add packages for NetInstall"]}];
                return;
            }
        }
    }
    ...
    if (
        ...
        if ( [certificatesArray count] != 0 ) {
            if ( ! [self addCopyCertificateScript:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying installCertificates.bash failed"]}];
                return;
            }
            ...
            if ( ! [self addCopyCertificates:certificatesArray error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying certificates failed"]}];
                return;
            }
        }
    }
    ...
    if (
        ...
        if ( [configurationProfilesArray count] != 0 ) {
            if ( ! [self addCopyConfigurationProfilesNetInstall:configurationProfilesArray error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying NetInstall configuration profiles failed"]}];
                return;
            }
        }
    }
    ...
    if (
        ...
        if ( [_userSettings[NBCSettingsUseBackgroundImageKey] boolValue] && ! [_userSettings[NBCSettingsBackgroundImageKey] isEqualToString:NBCBackgroundImageDefaultPath] ) {
            if ( ! [self addCopyDesktopPictureCustom:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying custom desktop picture failed"]}];
                return;
            }
        }
    }
    ...
    if ( _workflowType == kWorkflowTypeImagr ) {
        ...
        if (
            ...
            if ( ! [self addCopyDeployStudioAdmin:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying DeployStudio Admin.app failed"]}];
                return;
            }
        }
