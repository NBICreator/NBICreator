...
- (void)prepareResources:(NBCWorkflowItem *)workflowItem {
    ...
    if ( [creationTool length] != 0 ) {
        ...
        [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                            object:self
                                                          userInfo:@{ NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Unable to get creation tool"]}];
        return;
    }
...
- (void)prepareResourcesToInstall {
    ...
    if (
        ...
        if ( [packagesArray count] != 0 ) {
            if ( ! [self addInstallPackages:packagesArray error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing packages for installation failed"]}];
                return;
            }
        }
    }
    ...
    // --------------------------------------------------------------------------------
    if (
        _workflowType == kWorkflowTypeNetInstall
        ) {
        ...
        if ( [packagesArrayNetInstall count] != 0 ) {
            if ( ! [self addInstallPackagesNetInstall:packagesArrayNetInstall error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing NetInstall packages for installation failed"]}];
                return;
            }
        }
    }
...
- (void)prepareResourcesToCopy {
    ...
    if ( ( ! _isNBI && (
        ...
        if ( [certificatesArray count] != 0 ) {
            if ( ! [self addCopyCertificateScript:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing installCertificates.bash failed"]}];
                return;
            }
    ...
    if (
        ...
        if ( [configurationProfilesArray count] != 0 ) {
            if ( ! [self addCopyConfigurationProfilesNetInstall:configurationProfilesArray error:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing NetInstall configuration profiles failed"]}];
                return;
            }
        }
    }
    ...
    if ( ( ! _isNBI && (
        ...
        if ( [_userSettings[NBCSettingsUseBackgroundImageKey] boolValue] && ! [_userSettings[NBCSettingsBackgroundImageKey] isEqualToString:NBCBackgroundImageDefaultPath] ) {
            if ( ! [self addCopyDesktopPictureCustom:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing custom desktop picture failed"]}];
                return;
            }
        }
    }
    ...
    if ( _workflowType == kWorkflowTypeImagr ) {
        ...
        if ( ! [self addCopyCasperImaging:&error] ) {
            [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                object:self
                                                              userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing Casper Imaging.app failed"]}];
            return;
        }
        ...
        if (
            ...
            if ( ! [self addCopyDeployStudioAdmin:&error] ) {
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing DeployStudio Admin.app failed"]}];
                return;
            }
        }
...
- (void)imagrCopyError:(NSError *)error {
    ...
    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                        object:self
                                                      userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Copying Imagr.app failed"]}];
} // imagrCopyError
