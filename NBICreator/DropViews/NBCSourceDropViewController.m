...
- (void)selectSource:(NBCSource *)source creationTool:(NSString *)creationTool {
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        if ([sourceMenuName length] != 0) {
            if ([[self->_popUpButtonSource itemTitles] containsObject:sourceMenuName]) {
                ...
                [self->_popUpButtonSource selectItemWithTitle:NBCMenuItemNoSelection];
                [[[self->_popUpButtonSource menu] itemWithTitle:NBCMenuItemNoSelection] setState:NSMixedState];
            }
        } else {
...
- (void)updateSourceSelection:(NSString *)selection creationTool:(NSString *)creationTool {
    ...
    if ([creationTool length] != 0) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            [self->_popUpButtonSource selectItemWithTitle:selection];
        });
    } else {
