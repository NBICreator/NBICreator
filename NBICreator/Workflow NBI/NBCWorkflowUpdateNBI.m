    ...
    dispatch_async(taskQueue, ^{
        ...
        if ( [NBCDiskImageController attachBaseSystemDiskImageWithShadowFile:[target baseSystemURL] target:target error:&error] ) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                    object:self
                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Attaching BaseSystem disk image failed"] }];
            });
        }
    });
}
                ...
                if ( [NBCDiskImageController attachDiskImageAndReturnPropertyList:&netInstallDiskImageDict
                    ...
                    if ( netInstallDiskImageDict ) {
                        ...
                        if ( [netInstallVolumeURL checkResourceIsReachableAndReturnError:&error] ) {
                            ...
                            if ( netInstallDisk ) {
                                ...
                                dispatch_async(dispatch_get_main_queue(), ^{
                                    [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                                        object:self
                                                                                      userInfo:@{ NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Found no disk object matching disk image volume url"] }];
                                });
                                return;
                            }
                        } else {
                            dispatch_async(dispatch_get_main_queue(), ^{
                                [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                                    object:self
                                                                                  userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"NetInstall disk image volume url doesn't exist"] }];
                            });
                            return;
                        }
                    } else {
                        dispatch_async(dispatch_get_main_queue(), ^{
                            [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                                object:self
                                                                              userInfo:@{ NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Information dictionary returned from hdiutil was empty"] }];
                        });
                        return;
                    }
                } else {
            ...
            if ( [[target nbiNetInstallDisk] isMounted] ) {
                ...
                if ( ! [NBCDiskImageController detachDiskImageAtPath:[[target nbiNetInstallVolumeURL] path]] ) {
                    dispatch_async(dispatch_get_main_queue(), ^{
                        [[NSNotificationCenter defaultCenter] postNotificationName:NBCNotificationWorkflowFailed
                                                                            object:self
                                                                          userInfo:@{ NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Detaching NetInstall disk image failed"] }];
                    });
                    return;
                }
            }
