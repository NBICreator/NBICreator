...
- (void)createNBI:(NBCWorkflowItem *)workflowItem {
    ...
    if ([baseSystemDiskImageURL checkResourceIsReachableAndReturnError:&error]) {
        ...
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"BaseSystem disk image doesn't exist"] }];
        return;
    }
    ...
    if (![fm createDirectoryAtURL:temporaryNBIx86FolderURL withIntermediateDirectories:YES attributes:nil error:&error]) {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Unable to create temporary NBI folders"] }];
        return;
    }
      ...
      if ([fm copyItemAtURL:baseSystemDiskImageURL toURL:baseSystemTargetURL error:&blockError]) {
          ...
          dispatch_async(dispatch_get_main_queue(), ^{
            [nc postNotificationName:NBCNotificationWorkflowFailed
                              object:self
                            userInfo:@{
                                NBCUserInfoNSErrorKey : blockError ?: [NBCError errorWithDescription:@"BaseSystem disk image copy failed"]
                            }];
            return;
          });
      }
    ...
    // --------------------------------------------------------------------------
    [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(checkCopyProgressBaseSystem:) userInfo:nil repeats:YES];
}
    ...
    if ([NBCDiskImageController resizeAndMountBaseSystemWithShadow:baseSystemTemporaryURL target:[workflowItem target] error:&error]) {
        ...
        if ([baseSystemTemporaryVolumeURL checkResourceIsReachableAndReturnError:&error]) {
            ...
            if ([fm copyItemAtURL:booterSourceURL toURL:booterTargetURL error:&error]) {
                ...
                [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"booter copy failed"] }];
                return;
            }
            ...
            if (![fm copyItemAtURL:platformSupportSourceURL toURL:platformSupportTargetURL error:&error]) {
                [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"PlatformSupport.plist copy failed"] }];
                return;
            }
            ...
            if (![kernelCacheSourceURL checkResourceIsReachableAndReturnError:nil]) {
                ...
                if (![kernelCacheSourceURL checkResourceIsReachableAndReturnError:&error]) {
                    ...
                    [nc postNotificationName:NBCNotificationWorkflowFailed
                                      object:self
                                    userInfo:@{
                                        NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Found no prelinked kernelcache"]
                                    }];
                    return;
                }
            }
            ...
            if ([fm copyItemAtURL:kernelCacheSourceURL toURL:kernelCacheTargetURL error:&error]) {
                ...
                [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"kenelcache copy failed"] }];
                return;
            }
        } else {
            DDLogDebug(@"[DEBUG] NBI BaseSystem volume is NOT mounted");
            [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"NBI BaseSystem volume not mounted"] }];
            return;
        }
    } else {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Resizing NBI BaseSystem failed"] }];
        return;
    }
...
- (void)checkCopyProgressBaseSystem:(NSTimer *)timer {
    ...
    if ([fileAttributes count] != 0) {
        ...
        if (_temporaryNBIBaseSystemSize <= fileSize || _copyComplete == YES) {
            ...
            dispatch_async(dispatch_get_main_queue(), ^{
              [self->_delegate updateProgressStatus:[NSString stringWithFormat:@"Copying BaseSystem.dmg... %@/%@", fileSizeString, fileSizeOriginal] workflow:self];
              [self->_delegate updateProgressBar:(percentage * self->_progressOverallPercentage)];
            });
        }
    } else {
        [timer invalidate];
