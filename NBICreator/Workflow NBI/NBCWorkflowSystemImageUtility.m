...
- (void)createNBI:(NBCWorkflowItem *)workflowItem {
    ...
    if ([temporaryNBIURL checkResourceIsReachableAndReturnError:&error]) {
        ...
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Temporary NBI path not found"] }];
        return;
    }
    ...
    if (![self prepareWorkflowFolder:_temporaryNBIURL error:&error]) {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : error ?: [NBCError errorWithDescription:@"Preparing workflow folder failed"] }];
        return;
    }
...
- (BOOL)prepareWorkflowFolder:(NSURL *)workflowFolderURL error:(NSError **)error {
    ...
    if ([_workflowItem workflowType] == kWorkflowTypeNetInstall) {
        ...
        NSMutableArray *osInstallArray = [NSMutableArray arrayWithArray:@[ @"/System/Installation/Packages/OSInstall.mpkg", @"/System/Installation/Packages/OSInstall.mpkg" ]];
        ...
        if ([trustedNetBootServers count] != 0) {
            ...
            if (_packageOnly) {
                ...
                NSMutableString *additionalScriptsContent =
                    [[NSMutableString alloc] initWithContentsOfURL:additionalScriptsURL encoding:NSUTF8StringEncoding error:error] ?: [[NSMutableString alloc] init];
...
- (void)prepareWorkflowPackageOnly {
    ...
    if ([createRestoreFromSourcesArguments count] != 0) {
        ...
        [nc postNotificationName:NBCNotificationWorkflowFailed
                          object:self
                        userInfo:@{
                            NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Creating script arguments for createRestoreFromSources.sh failed"]
                        }];
        return;
    }
    ...
    if (![NBCWorkflowNBIController generateEnvironmentVariablesForCreateRestoreFromSources:_workflowItem]) {
        [nc postNotificationName:NBCNotificationWorkflowFailed
                          object:self
                        userInfo:@{
                            NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Creating environment variables for createRestoreFromSources.sh failed"]
                        }];
        return;
    }
    ...
    if (![@{ @"packageOnlyMode" : @YES } writeToURL:installPreferencesPlistURL atomically:NO]) {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Writing InstallPreferences.plist failed"] }];
        return;
    }
    ...
    if ([asrInstallPkgSourceURL checkResourceIsReachableAndReturnError:&err]) {
        ...
        if (![fm copyItemAtURL:asrInstallPkgSourceURL toURL:asrInstallPkgTargetURL error:&err]) {
            [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"Copying ASRInstall.pkg failed"] }];
            return;
        }
    } else {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"ASRInstall.pkg doesn't exist"] }];
        return;
    }
    ...
    if ([asrPostInstallPackagesURL checkResourceIsReachableAndReturnError:&err]) {
        ...
        if (![fm copyItemAtURL:asrPostInstallPackagesURL toURL:asrPostInstallPackagesTargetURL error:&err]) {
            [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"Copying postInstallPackages.sh failed"] }];
            return;
        }
    } else {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"postInstallPackages.sh doesn't exist"] }];
        return;
    }
    ...
    if ([preserveInstallLogURL checkResourceIsReachableAndReturnError:&err]) {
        ...
        if (![fm copyItemAtURL:preserveInstallLogURL toURL:preserveInstallLogTargetURL error:&err]) {
            [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"Copying reserveInstallLog.sh failed"] }];
            return;
        }
    } else {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"reserveInstallLog.sh doesn't exist"] }];
        return;
    }
    ...
    if ([netBootClientHelperURL checkResourceIsReachableAndReturnError:&err]) {
        ...
        if (![fm copyItemAtURL:netBootClientHelperURL toURL:netBootClientHelperTargetURL error:&err]) {
            [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"Copying NetBootClientHelper failed"] }];
            return;
        }
    } else {
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"NetBootClientHelper doesn't exist"] }];
        return;
    }
...
- (void)runWorkflowPackageOnlyWithArguments:(NSArray *)arguments {
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] createRestoreFromSourcesWithArguments:arguments
...
- (void)runWorkflowNetInstall {
    ...
    if ([installESDVolumeURL checkResourceIsReachableAndReturnError:&err]) {
        ...
        [nc postNotificationName:NBCNotificationWorkflowFailed object:self userInfo:@{ NBCUserInfoNSErrorKey : err ?: [NBCError errorWithDescription:@"InstallESD disk image is not mounted"] }];
        return;
    }
    ...
    if ([createNetInstallArguments count] != 0) {
        ...
        [nc postNotificationName:NBCNotificationWorkflowFailed
                          object:self
                        userInfo:@{
                            NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Creating script arguments for createNetInstall.sh failed"]
                        }];
        return;
    }
    ...
    if (![NBCWorkflowNBIController generateEnvironmentVariablesForCreateNetInstall:_workflowItem]) {
        [nc postNotificationName:NBCNotificationWorkflowFailed
                          object:self
                        userInfo:@{
                            NBCUserInfoNSErrorKey : [NBCError errorWithDescription:@"Creating environment variables for createNetInstall.sh failed"]
                        }];
        return;
    }
    ...
    dispatch_async(taskQueue, ^{
      ...
      }] createNetInstallWithArguments:arguments
                          authorization:authData
                              withReply:^(NSError *error, int terminationStatus) {
                                if (terminationStatus == 0) {
                                    dispatch_async(dispatch_get_main_queue(), ^{
...
- (void)updateProgressBarCopy {
    ...
    // ---------------------------------------------------
    [NSTimer scheduledTimerWithTimeInterval:0.5 target:self selector:@selector(checkCopyProgress:) userInfo:nil repeats:YES];
